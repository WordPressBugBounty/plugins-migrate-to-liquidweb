...
if (!class_exists('LWHelper')) :
	class LWHelper {
		...
		public static function fileRemovePattern($fname, $pattern, $is_regex = false) {
			...
			if (!LWWPFileSystem::getInstance()->exists($fname)) {
				return;
			}

			$content = LWWPFileSystem::getInstance()->getContents($fname);
			if ($content !== false) {
				if ($is_regex !== false) {
				...
				if ($content !== $modified_content) {
					LWWPFileSystem::getInstance()->putContents($fname, $modified_content,
							LWWPFileSystem::getInstance()->getchmodOctal($fname));
				}
			}
		}
