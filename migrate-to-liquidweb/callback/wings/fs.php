...
if (!class_exists('LWFSCallback')) :
require_once dirname( __FILE__ ) . '/../streams.php';

class LWFSCallback extends LWCallbackBase {
	public $stream;
	public $account;
	...
	function getFilesContent($files, $withContent = true) {
		...
		foreach ($files as $file) {
			...
			if ((LWWPFileSystem::getInstance()->isDir($absfile) === true) && !is_link($absfile)) {
				$fdata['is_dir'] = true;
			} else {
				if (isset($fdata["error"]) && $fdata["error"] === "NOT_READABLE") {
					...
					if ($withContent === true) {
						$content = LWWPFileSystem::getInstance()->getContents($absfile);
						if ($content !== false) {
	...
	function uploadFiles($files, $offset = 0, $limit = 0, $bsize = 102400) {
		$result = array();
		foreach ($files as $file) {
			if (LWWPFileSystem::getInstance()->isReadable(ABSPATH.$file) === false) {
				$result["missingfiles"][] = $file;
				continue;
			}
	...
	function process($request) {
		...
		$stream_init_info = LWStream::startStream($this->account, $request);
