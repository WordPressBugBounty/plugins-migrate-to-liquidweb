...
if (!class_exists('LWFSWriteCallback')) :

class LWFSWriteCallback extends LWCallbackBase {
	...
	public function removeDirs($dirs) {
		...
		foreach ($dirs as $dir) {
			...
			if ((LWWPFileSystem::getInstance()->isDir($dir) === true) && !is_link($dir)) {
				if ($this->isEmptyDir($dir)) {
					$dir_result['status'] = LWWPFileSystem::getInstance()->rmdir($dir);
					if ($dir_result['status'] === false) {
						$dir_result['error'] = "RMDIR_FAILED";
						$fs_error = LWWPFileSystem::getInstance()->checkForErrors();
						if (isset($fs_error)) {
	...
	public function doChmod($path_infos) {
		...
		foreach ($path_infos as $path => $mode) {
			...
			if (LWWPFileSystem::getInstance()->exists($path) === true) {
				$path_result['status'] = LWWPFileSystem::getInstance()->chmod($path, $mode);
	...
	public function renameFiles($path_infos) {
		...
		foreach ($path_infos as $oldpath => $newpath) {
			...
			if (LWWPFileSystem::getInstance()->exists($oldpath)) {
				$action_result['status'] = LWWPFileSystem::getInstance()->move($oldpath, $newpath, true);
	...
	public function writeContentToFile($content, $ofile) {
		...
		if (LWWPFileSystem::getInstance()->putContents($ofile, $content) === false) {
			$result['error'] = 'UNABLE_TO_WRITE_TO_TMP_OFILE';
			$fs_error = LWWPFileSystem::getInstance()->checkForErrors();
			if (isset($fs_error)) {
	...
	public function moveUploadedFile($ofile) {
		...
		if (isset($_FILES['myfile'])) {
			...
			if ($is_upload_ok) {
				...
				if (LWWPFileSystem::getInstance()->move($tmp_name, $ofile, true) === false) {
	...
	public function process($request) {
		...
		case "fleops":
			...
			break;
		case "cncatfls":
			$bsize = (isset($params['bsize'])) ? $params['bsize'] : (8 * LWFSWriteCallback::MEGABYTE);
