...
if (!class_exists('LWRespStream')) :

	class LWStream extends LWCallbackBase {
		public $bvb64stream;
		public $bvb64cksize;
		public $checksum;
		...
		public static function startStream($account, $request) {
			...
			$stream = new LWRespStream($request);
			if ($request->isAPICall()) {
				$stream = new LWHttpStream($request);
				if (!$stream->connect()) {
					$apicallstatus = array(
...
class LWRespStream extends LWStream {
	public $bvboundry;
...
class LWHttpStream extends LWStream {
	var $user_agent = 'LWHttpStream';
	var $host;
	var $port;
	var $timeout = 20;
	var $conn;
