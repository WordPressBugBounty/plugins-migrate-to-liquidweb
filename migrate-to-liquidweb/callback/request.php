...
if (!class_exists('LWCallbackRequest')) :
	class LWCallbackRequest {
		public $params;
		public $method;
		public $wing;
		public $is_afterload;
		...
		public function __construct($account, $in_params, $settings) {
			...
			$this->pubkey_name = !empty($in_params['pubkeyname']) ?
					LWAccount::sanitizeKey($in_params['pubkeyname']) : 'm_public';
			$this->bvprmsmac = !empty($in_params['bvprmsmac']) ? LWAccount::sanitizeKey($in_params['bvprmsmac']) : "";
			...
			if (array_key_exists('bvprms', $in_params) && isset($in_params['bvprms'])) {
				...
				if ($this->compare_mac($this->bvprmsmac, $calculated_mac) === true) {
					...
					if (array_key_exists('sersafe', $in_params)) {
						...
						$in_params[$key] = LWCallbackRequest::serialization_safe_decode($in_params[$key]);
					}
		...
		public static function serialization_safe_decode($data) {
			if (is_array($data)) {
				$data = array_map(array('LWCallbackRequest', 'serialization_safe_decode'), $data);
		...
		public function verify($data, $sig, $sighshalgo) {
			...
			$public_key_str = LWWPFileSystem::getInstance()->getContents($key_file);
		...
		public function corruptedParamsResp() {
			$bvinfo = new LWInfo($this->settings);
		...
		public function authFailedResp() {
			$api_public_key = LWAccount::getApiPublicKey($this->settings);
			$default_account_pubkey = LWAccount::getDefaultPublicKey();
			$bvinfo = new LWInfo($this->settings);
			$resp = array(
				"request_info" => $this->info(),
				"bvinfo" => $bvinfo->info(),
