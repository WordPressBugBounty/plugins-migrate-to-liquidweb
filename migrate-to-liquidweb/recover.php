...
if (!class_exists('LWRecover')) :
	class LWRecover {
		...
		public static function defaultSecret($settings) {
			...
			if (empty($secret)) {
				$secret = LWRecover::refreshDefaultSecret($settings);
			}
			return $secret;
		}
		...
		public static function refreshDefaultSecret($settings) {
			...
			$key_details = array();
			$key_details["key"] = LWAccount::randString(32);
			...
			if (empty(self::configSalt())) {
				$key_details["salt"] = LWAccount::randString(self::SALT_LENGTH);
			}
		...
		public static function find($settings, $pubkey, $tag = null) {
			...
			$account = new LWAccount($settings, $pubkey, $secret);
			return $account;
		}
	}
endif;
