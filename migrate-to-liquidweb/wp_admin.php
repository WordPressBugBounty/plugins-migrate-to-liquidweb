...
class LWWPAdmin {
	...
	function removeAdminNotices() {
		if (LWHelper::getRawParam('REQUEST', 'page') === $this->bvinfo->plugname) {
	...
	public function lwsecAdminMenu($hook) {
		if ($hook === 'toplevel_page_liquidweb') {
			wp_enqueue_style('lwsurface', plugins_url('css/style.css', __FILE__), array(), $this->bvinfo->version);
		}
	}
	...
	public function siteInfoTags() {
		...
		$ctag = LWRecover::connectionTag($this->settings);
		$public = LWAccount::getApiPublicKey($this->settings);
		$server_ip = LWHelper::getStringParamEscaped('SERVER', 'SERVER_ADDR', 'attr');
